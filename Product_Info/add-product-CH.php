...
if($product_img["error"]==0) {
    if(move_uploaded_file($_FILES["product_img"]["tmp_name"], "images-CH/".$_FILES["product_img"]["name"])){
        ...
        $sql = "INSERT INTO product_info (product_name,category_id, product_introduce, product_spec, product_price, product_serial, product_amount, product_img, created_at, updated_at, valid) VALUES ('$product_name', '$product_category','$product_introduce', '$product_spec', '$product_price', '$product_serial', '$product_amount', '$product_img_name', '$now', '$now', '1')";
